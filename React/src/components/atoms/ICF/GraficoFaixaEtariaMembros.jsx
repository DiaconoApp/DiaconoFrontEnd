...
import { getFaixaEtariaMembros } from "../../../services/dashboards";

const cores = ["#10b981", "#059669", "#047857", "#065f46"];

export default function GraficoFaixaEtariaMembros({ anoInicio, anoFim }) {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        setCarregando(true);
        const res = await getFaixaEtariaMembros(anoInicio, anoFim);
        
        // Formata os dados para o Recharts
        const formatado = res.map((item) => ({
          name: item.faixa || item.faixaEtaria,
          value: item.quantidade || item.quantidadeMembros,
        }));

        setDados(formatado);
      } catch (e) {
        console.error("Erro ao carregar distribuição por faixa etária:", e);
        setDados([]);
      } finally {
        setCarregando(false);
      }
  ...
  return (
    <div className="bg-white shadow-sm p-5 rounded-xl h-full flex flex-col">
      <h2 className="font-semibold text-icf-primary-400 mb-4">Faixa Etária</h2>
