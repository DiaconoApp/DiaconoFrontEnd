...
const FAQ = () => {
  ...
  return (
    <section id="faq" className="pt-16 sm:pt-20 md:pt-28 pb-16 sm:pb-20 md:pb-28 relative overflow-hidden bg-ice-white border-none" ref={ref}>
      ...
      <div 
        className="hidden md:block absolute w-[600px] h-[600px] bg-diacono-blue-400/8 rounded-full blur-[80px] pointer-events-none transition-all duration-300 ease-out"
        style={{
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className={`bg-white rounded-xl border border-diacono-blue-100 hover:border-diacono-blue-300/30 shadow-sm hover:shadow-md transition-all duration-700 px-6 overflow-hidden ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <AccordionTrigger className="text-base font-semibold text-diacono-blue-400 hover:no-underline">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-sm text-diacono-blue-300/70 leading-relaxed">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
